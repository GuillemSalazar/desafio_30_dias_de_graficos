...
# Graficar
g<-ggplot() +
  geom_bar(data = toplot_inner_ring,aes(x=5,y=n,fill=personaje),stat = "identity",color = "white",width=1,alpha=0.6) +
  geom_bar(data = toplot_outer_ring,aes(x=6,y=n,fill=personaje),stat = "identity",color = "white",width=1,alpha=0.6) +
  geom_text(data=toplot_inner_ring,aes(x=5,y = lab.ypos, label = label),size=2.3,fontface="bold",hjust=0.5) +
